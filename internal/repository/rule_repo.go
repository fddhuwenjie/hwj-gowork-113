...
func (r *RuleRepo) Activate(ctx context.Context, q store.Queryer, id string, now time.Time) error {
	...
	if n == 0 {
		return apperr.Statef("规则版本 %s 非草稿状态，不能启用", id)
	}
	return nil
}
...
func (r *RuleRepo) ActiveByCode(ctx context.Context, q store.Queryer, code string) (*domain.RuleVersion, error) {
	row := q.QueryRowContext(ctx, `SELECT `+ruleCols+` FROM rule_versions WHERE code=? AND status IN ('ACTIVE','RETIRED') ORDER BY version_no DESC LIMIT 1`, code)
	return scanRule(row)
}
