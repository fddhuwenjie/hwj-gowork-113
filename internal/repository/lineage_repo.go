...
func (r *LineageRepo) ListParents(ctx context.Context, q store.Queryer, childBatchID string) ([]domain.LineageEdge, error) {
	return r.query(ctx, q, `SELECT e.id, e.resource_id, e.parent_batch_id, e.child_batch_id, e.relation, e.created_at
		FROM lineage_edges e WHERE e.resource_id = (
			SELECT a.resource_id FROM batches b JOIN accessions a ON a.id=b.accession_id WHERE b.id=?
		) ORDER BY e.created_at, e.id`, childBatchID)
}
