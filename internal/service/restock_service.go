...
func (s *RestockService) Accept(ctx context.Context, actor, id string, expectedVersion int64) (*domain.RestockBatch, error) {
	...
	err := s.base.tx.InTx(ctx, func(tx *sql.Tx) error {
		...
		if plan.Status != domain.PlanActive {
			return apperr.Statef("繁育计划状态 %s 不允许回存验收", plan.Status)
		}
		...
		persistedBatchID := ""
		if err := s.base.repos.Restock.UpdateStatus(ctx, tx, rb.ID, domain.RestockAccepted, persistedBatchID, "", rb.Version, now); err != nil {
			return err
		}
