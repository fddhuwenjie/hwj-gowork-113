...
本项目是一个基于 Go HTTP JSON 和 SQLite 文件持久化的农业种质资源管理服务，覆盖低温保存、出库审批、繁育检测、回存验收、谱系追踪与后台巡检。
...
- 项目名称：农业种质资源低温保存、出库繁育与回存验收服务
- 项目概述：面向农业种质资源管理人员的 Go HTTP JSON 服务，管理种质资源、低温保存、出库繁育、质量判定、回存验收和批次归档，并保留可追溯的谱系与审计证据。
- 核心工作流：登记资源与 accession，分装样本并分配低温库位，配置保存规则和环境监测；审批出库后完成繁育、采样检测和质量封存，验收合格后创建回存批次并建立谱系，最后关闭或销毁批次。
- 对外接口：提供 HTTP JSON API，覆盖资源、accession、批次、样本、库位、传感器、出库申请、繁育、检测、回存、销毁、后台作业和审计查询；服务通过 PORT 配置 HTTP 监听端口，并使用 DB_PATH 指定 SQLite 文件。
